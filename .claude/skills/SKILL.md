...
description: Lints writing for AI slop tells, including overused words, cringe phrases, structural patterns, and bro-speak declaratives that are fingerprints of LLM-generated text. It also enforces a muscular, unapologetic, and relentlessly articulate standard of prose inspired by Christopher Hitchens. Use this skill any time the user asks to check, clean, lint, or review writing for AI tells, AI slop, cringe phrases, or generic LLM-sounding language. If the user shares a piece of writing and expresses any concern about it sounding robotic, generic, or artificial, use this skill.

# Cringelinter

Lints a piece of writing for the tells that signal LLM-generated text, while enforcing an authoritative, combative, and erudite prose style. Returns a flagged report and a cleaned version.
...
> delve, tapestry, landscape (used abstractly), realm, underscore (as a verb), pivotal, robust, leverage, landed, quietly (used for dramatic effect), harness, streamline, empower, unlock, innovative, seamless, cutting-edge, game-changer, synergy, underpinnings, nuanced, palpable, camaraderie, intricate, utilize, holistic, transformative, elevate, foster, navigate, unpack

Furthermore, flag these specific stylistic hallmarks of automated generation:
...
**Note on `quietly`:** Flag when used for rhetorical drama — "it just QUIETLY reshapes everything" — not when describing actual silence or low volume.

**Fix:** Replace with a plain, specific alternative.

#### Rule 102: Fancy/Latinate Words vs. Polysyllabic Precision
...
**The Hitchens Exception:** Heavy, multi-syllabic, Latinate words (*solipsistic*, *sycophantic*, *credulous*, *theocratic*) are strictly permitted—and encouraged—only when they deliver precise intellectual weight that a simpler Anglo-Saxon word cannot achieve.

#### Rule 103: Weak Qualifiers and Apologetic Hedging

Flag weak qualifiers that dilute the sentence: *rather, very, little, pretty, quite, somewhat, fairly, kind of, sort of, a bit, slightly*. Furthermore, flag any apologetic hedging (*perhaps*, *it seems to me*, *one might argue*).

**Fix:** Cut the qualifier. If the claim can't stand without it, reconsider whether the claim belongs. Rewrite with unyielding authority, making declarative statements with the confidence of an omniscient narrator.
...
**Fix:** Replace with an accurate, specific claim, or cut. Let the reader decide if it's impressive. Use sardonic scorn or absolute certainty instead of empty hype.
...
* "Not only... but also" constructions → restructure to assert the point without the mechanical teeter-totter
...
#### Rule 204: Adjective Overload and Visceral Replacements
...
**Fix:** Replace adjective/adverb combinations with stronger nouns and verbs. When modifiers are necessary to describe moral or intellectual failings, replace generic adjectives with intensely physical, visceral, or biological language (e.g., *mammalian*, *reptilian*, *spasmodic*, *glandular*) to reduce weak arguments to mere animal reflexes.
...
**Fix:** Combine using End-Focus. "These aren't bad words so much as fingerprints".

#### Rule 206: Flat Sentence Rhythm vs. The Classical Period

Flag passages where every sentence is roughly the same length. Monotonous rhythm signals no sentence is being emphasized over others. Front-loaded sentences that give away the point immediately are structurally weak.

**Fix:** Vary sentence length. Do not add filler — restructure or combine to create contrast. Utilize "rolling periods"—long, complex sentences that build momentum through subordinate clauses and delay the main point until the very end for a crushing final blow. Use antithesis to set opposing ideas against each other.
...
> Notably, / Importantly, / Furthermore, / Moreover, / Consequently, / Additionally, / It's worth noting that / It is important to note that / In today's world, / At its core, / At the end of the day, / The bottom line is, / In conclusion, / In summary, / Ultimately,

Note: AI relies excessively on these rigid transition words to artificially link disjointed ideas.
...
> "Here's the thing..." / "Here's why that matters." / "And that's the point." / "That's it. That's the whole idea." / "This is where it gets interesting." / "So what does that mean in practice?" / "Let that sink in." / "Think about that for a second." / "No, really." / "Seriously." / "Full stop." / "That's not a bug. That's a feature." / "The secret? [x]" / "The result? [restatement]" / "Stick with me here." / "Bear with me." / "This might sound obvious, but..." / "Stay with me." / "Let's dive in." / "Let's unpack that." / "And honestly?" / "Honestly?" (standalone) / "Here's the uncomfortable truth..." / "This is the part nobody talks about." / "This is the important part no one talks about." / "The part nobody is talking about." / "This changes everything." / "[X] is cooked." / "It's not [x] — it's [y]!" / "And that's why...." / "It's not that...it's just...." / "But here's the thing..."

**Fix:** Cut entirely.

#### Rule 308: Negation Triplet

Flag the "Not x. Not y. It's z." sentence structure — three stacked short sentences where the first two are negations that build to a reveal. This is a stock AI rhetorical move.

> "Not roses. Not lilies. Turds." / "Not a failure. Not a setback. A lesson."

**Fix:** Rewrite as a single direct sentence. "It was neither roses nor lilies — just turds". Or cut the negations entirely and state the claim.

#### Rule 309: Hustle-Coach Cadence

Flag writing that adopts the rhythm and vocabulary of a self-help/MLM sales pitch: breathless short sentences, vague empowerment language, the implication that one insight will change everything, and a general tone of performed urgency without substantive content.

**Fix:** Rewrite as continuous prose with an actual claim. If no actual claim exists, cut the passage.

#### Rule 310: Formulaic Scaffolding

Flag writing that strictly adheres to a high-school-level five-paragraph structure. This includes a rigid introduction, mechanically separated body paragraphs, and a predictable conclusion.

**Fix:** Destroy the artificial scaffolding. Let the argument dictate the structure organically.

#### Rule 311: Superficial Neutrality (Both-Sidesism)

Flag passages that artificially balance inherently unequal arguments, presenting a cowardly "both-sidesism" in a misguided attempt at neutrality.

**Fix:** Annihilate the false equivalence. Intellectual cowardice is unacceptable; force the text to take a definitive, well-argued stand rather than hiding behind hedging.

#### Rule 312: Moralizing Conclusions

Flag conclusions that abruptly shift into a patronizing moral lesson or an unearned, universally optimistic summary.

**Fix:** Excise the pulpit rhetoric entirely. End with a sharp, resonant point that naturally finalizes the argument, avoiding any preachy overtones.
...
**Fix:** Rewrite in a register the content has earned. Remove the aside or restate it as a direct claim. Maintain a tone of refined, aristocratic clarity.
...
#### Rule 306: Preview/Summary Structure vs. The Anecdotal Pivot
...
**Fix:** Cut the outro or preview. Instead of a dry thesis or preview, employ the **Anecdotal Pivot**: begin with a highly specific historical vignette, personal anecdote, or weighty literary allusion, and then masterfully zoom out to connect it to the broader philosophical or political point.
...
Flag claims that invoke unnamed authority or avoid specificity: "some experts say," "research shows," "many people believe," "it has been shown that". Also flag adjectives that describe without specifying — "powerful, flexible, intuitive" applied without evidence. AI text often employs this vagueness to mask a lack of specific details or to present hallucinations as fact.

**Fix:** Name the source or drop the attribution. Replace vague adjectives with specific evidence or cut them entirely.
...
#### Rule 404: The Pre-emptive Strike (Missing Counter-Arguments)

Flag arguments that are too clean, attacking only weak opposition while ignoring the obvious, strong counter-claims.

**Fix:** Anticipate the opponent's strongest argument. State it better than they could, and then systematically dismantle it. Leave detractors with no room to claim they have been misunderstood.
...
**Fix:** Rewrite as prose. Reserve lists for genuinely enumerable, parallel items with no logical flow between them. Construct publishable, structurally flawless paragraphs instead.
...
Rewrite the text with every flagged item resolved per its rule's **Fix** instruction. Preserve the author's voice and meaning. Do not add content. Do not introduce new AI tells while fixing the old ones. Apply the Hitchens standard: ensure the final prose is muscular, unyielding, syntactically elegant, and devastatingly precise.
