var allQuotes=[];var quoteIndex=0;var autoTimer=null;function initQuotes(){fetch('data/quotes.json').then(function(r){return r.json();}).then(function(data){allQuotes=shuffle(data);showQuote(0);initSwipe();}).catch(function(){});}
...
function nextQuote(){if(autoTimer)clearTimeout(autoTimer);quoteIndex=(quoteIndex+1)%allQuotes.length;showQuote(quoteIndex);}
function prevQuote(){if(autoTimer)clearTimeout(autoTimer);quoteIndex=(quoteIndex-1+allQuotes.length)%allQuotes.length;showQuote(quoteIndex);}
...
var touchStartX=0;var touchEndX=0;var SWIPE_THRESHOLD=50;function initSwipe(){var el=document.getElementById('quotes');if(!el)return;el.addEventListener('touchstart',function(e){touchStartX=e.changedTouches[0].screenX;},{passive:true});el.addEventListener('touchend',function(e){touchEndX=e.changedTouches[0].screenX;var diff=touchStartX-touchEndX;if(Math.abs(diff)>SWIPE_THRESHOLD){if(diff>0){nextQuote();}
else{prevQuote();}}},{passive:true});}
function shuffle(arr){var a=arr.slice();for(var i=a.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1));var t=a[i];a[i]=a[j];a[j]=t;}
return a;}